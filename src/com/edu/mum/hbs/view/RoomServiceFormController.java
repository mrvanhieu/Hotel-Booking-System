...
public class RoomServiceFormController extends ControllerBase {
	...
	@FXML	private ChoiceBox<String> serviceDesc;
	...
	public void initialize(URL location, ResourceBundle resources) {
		...
		roomServiceTable.getSelectionModel().selectedItemProperty().addListener(new ChangeListener<RoomService>() {
			@Override
			public void changed(ObservableValue<? extends RoomService> observable, RoomService oldValue, RoomService newValue) {
	...
	@FXML	public void addRoomService() {
		if (roomNumber.getValue() == null || serviceDesc.getValue() == null
				|| quantity.getValue() == null || serviceDate.getValue() == null
				|| roomNumber.getValue().isEmpty() || serviceDesc.getValue().isEmpty()
	...
	@FXML	public void updateRoomService() {
		RoomService roomServiceSelected = roomServiceTable.getSelectionModel().getSelectedItem();
        if (roomServiceSelected == null) { 
			showAlert(AlertType.INFORMATION, "Select room service", null, "You have to select a room's service in table to continue.");
        	return;		
        }
        ...
        if (!roomServiceSelected.getRoomNumber().equals(roomNumber.getValue())
        	...
        	showAlert(AlertType.INFORMATION, "Wrong values are entered", null, "You can only change Quantity value for updating.");
	...
	@FXML	public void deleteRoomService() {
		RoomService roomServiceSelected = roomServiceTable.getSelectionModel().getSelectedItem();
        if (roomServiceSelected == null) { 
			showAlert(AlertType.INFORMATION, "Select room service", null, "You have to select a room's service in table to continue.");
        	return;		
